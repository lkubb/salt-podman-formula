...
We will see a quick setup to configure the ``podman`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``podman`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``podman`` formulas under ``/srv/salt/podman/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/podman/parameters/dns:domain/
    ln -s dns:domain /srv/salt/podman/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/podman/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/podman/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``podman`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``podman`` formulas under ``/srv/salt/podman/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/podman/parameters/roles
...
We create a configuration for the role ``podman/server`` in ``/srv/salt/podman/parameters/roles/podman/server.yaml``:
...
We create another configuration for the role ``podman/client`` in ``/srv/salt/podman/parameters/roles/podman/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles podman/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles podman/client
...
.. code-block:: console
    ...
    mv /srv/salt/podman/parameters/dns:domain/  '/srv/salt/podman/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-podman-as-json:
      test.nop:
        - name: {{ podman | json }}
...
Here is an example based on `template-formula/podman/config/file.sls`_:
...
This ``sls`` file expose a ``podman`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ podman['winner'] }}
...
.. _template-formula/podman/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/podman/config/file.sls
